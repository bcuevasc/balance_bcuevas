// ==========================================================
// 🧠 BÚNKER SCADA - MOTOR LÓGICO V7.6.2 (ESTABLE Y BLINDADO)
// ==========================================================
...
document.addEventListener("DOMContentLoaded", () => {
    const optionsHTML = catMaestras.map(c => `<option value="${c.id}">${c.em} ${c.id}</option>`).join('');
...
function actualizarDashboard() {
    ...
    [...dataMes].sort((x,y) => x.fechaISO < y.fechaISO ? -1 : 1).forEach(x => {
        if (x.esIn) { tI += x.monto; saldoAcc += x.monto; }
            ...
            tEvitable += (x.monto * (x.innecesarioPct / 100));
        }
    });
    ...
    datos.forEach((x, idx) => {
        ...
        if (contenedorPC) {
            htmlPC += `<tr style="${bgEdicion}" draggable="true" ondragstart="dragStart(event, '${x.firestoreId}')" ondragover="dragOver(event)" ondragleave="dragLeave(event)" ondrop="dropRow(event, '${x.firestoreId}')">
                ...
                <td class="hide-mobile" style="text-align:center; font-size:0.7rem; color:var(--text-muted);">${x.innecesarioPct || 0}%</td>
...
function dibujarGraficos(sueldo, chronData, cats, diasCiclo, T0) {
    ...
    if(sIn && sFin) { sIn.max = diasCiclo; sIn.value = 0; sFin.max = diasCiclo; sFin.value = diasCiclo; }
    ...
    // Auto-calculate zoom stats on load
    hacerZoomGrafico(0, diasCiclo);
}
