// ==========================================================
// 🌐 V15.0: MOTOR SCADA PRO (Compact & Sync Edition)
// ==========================================================
...
function actualizarDashboard() {
    ...
    if(document.getElementById('barFijos')) document.getElementById('barFijos').style.width = Math.min((tF / (sueldoOperativoBase || 1)) * 100, 100) + "%";
    if(document.getElementById('barOtros')) document.getElementById('barOtros').style.width = Math.min((tO / (sueldoOperativoBase || 1)) * 100, 100) + "%";
    ...
    if(txtPctFugas) { txtPctFugas.innerText = pctFugas + '%'; txtPctFugas.style.color = pctFugas < 5 ? "var(--color-saldo)" : (pctFugas <= 10 ? "#d29922" : "var(--color-fuga)"); }

    let deudaAprox = window.totalTC || 0; 
    const txtProyectado = document.getElementById('txtProyectado');
    if(txtProyectado) { let proyVal = saldoAcc - deudaAprox; txtProyectado.innerText = proyVal.toLocaleString('es-CL'); txtProyectado.style.color = proyVal < 0 ? "var(--color-fuga)" : "#79c0ff"; }
    ...
    if (document.getElementById('txtGastoTotalPeriodo')) document.getElementById('txtGastoTotalPeriodo').innerText = '$' + (tO + tF).toLocaleString('es-CL');
    setTxt('txtGastoTramo', tO + tF); setTxt('txtPromedioZoom', Math.round((tO + tF) / diasCiclo));
}

// 📌 RENDERIZADO DE TABLA DE GASTOS COMPACTA
window.renderizarListas = function(sueldoBase, filtroBuscador) {
    ...
    const contenedorPC = document.getElementById('listaDetalle'); if (!contenedorPC) return;
    if(datos.length === 0) return contenedorPC.innerHTML = `<tr><td colspan="7" style="text-align:center; padding:20px; color:var(--text-muted);">MATRIZ SIN DATOS</td></tr>`;
    ...
    datos.forEach((x) => {
        ...
        htmlPC += `<tr style="${bgEdicion}">
            ...
            <td style="color:var(--text-muted);">${d.toLocaleDateString('es-CL', {day:'2-digit', month:'2-digit'})} <span class="col-hora">${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}</span></td>
            <td class="col-desc" title="${x.nombre || 'N/A'}">${x.nombre || 'N/A'}</td>
            <td><span class="cat-badge">${x.catV.replace(' & ','&')}</span></td>
...
// 🖌️ FUNCIONES DE GESTIÓN Y EDICIÓN
...
function editarMovimiento(id) {
    ...
    const inputNombre = document.getElementById('inputNombre');
    if(inputNombre) { inputNombre.value = mov.nombre; if (window.innerWidth > 768) { inputNombre.focus(); setTimeout(() => inputNombre.select(), 50); } }
...
window.agregarMovimiento = function() {
    ...
    op.then(() => { limpiarFormulario(); mostrarToast("REGISTRO CONFIRMADO"); }).catch(err => { alert("Error: " + err.message); btn.innerHTML = "ERROR"; btn.disabled = false; });
}
...
// 📊 COMPARTIMENTACIÓN GRÁFICA & ETIQUETAS DE DATOS (DATA LABELS)
function dibujarGraficos(sueldo, chronData, cats, diasCiclo, T0, totalFijosMes, tInfra, tFlota, deudaAprox) {
    ...
    // ⚡ FIX: ETIQUETAS VISIBLES EN GRÁFICOS (Barras y Líneas)
    const labelsPlugin = {
        id: 'labelsPlugin',
...
// 🚀 SIMULADOR DÍA CERO Y PRE-VUELO (Lógica Temporal +1 Aplicada)
...
// ☁️ SYNC Y EXPORTACIÓN LEGACY (Punto 6)
